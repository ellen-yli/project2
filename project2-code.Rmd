...
recipe.search<- function(key=NULL, query=NULL, diet=NULL, type=NULL, maxCarbs=NULL, minProtein=NULL, maxFat=NULL){
  ...
  search.opt<-list(query=str_remove_all(query, " "), diet=str_remove_all(diet, " "), type=str_remove_all(type, " "), maxCarbs=maxCarbs, minProtein=minProtein, maxFat=maxFat, apiKey=key)
...
random.recipes <- function(apiKey, number=NULL, tags=NULL){
  if(is.null(apiKey)){
    return("Please enter your API key.")
  }
  
  baseURL <- "https://api.spoonacular.com/recipes/random"
  input <- list(number=number, tags=str_remove_all(tags, " "))
...
## `diet.restrictions`
For people with dietary restrictions, intolerances, or sensitivities, this function allows you to specify [diet](https://spoonacular.com/food-api/docs#Diets), food [intolerances](https://spoonacular.com/food-api/docs#Intolerances), and also allows for ingredients to be excluded. To give more than one option, you will need to give a string of comma separated values.
```{r}
#function to search for recipes given diet type, food intolerances, excluded ingredients
diet.restrictions <- function(apiKey, diet=NULL, intolerances=NULL, exclude=NULL){
  if(is.null(apiKey)){
    return("Please enter your API key.")
  }
  
  baseURL <- "https://api.spoonacular.com/recipes/complexSearch"
  input <- list(diet = str_remove_all(diet, " "), intolerances = str_remove_all(intolerances, " "), excludeIngredients = str_remove_all(exclude, " "))
  opts <- input[-which(sapply(input, is.null))]
  link <- paste0(baseURL, "?apiKey=", apiKey)
  for(i in 1:length(opts)){
    link<-paste0(link,"&", names(opts[i]), "=", opts[i])
